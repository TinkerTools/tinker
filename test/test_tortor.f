...
c     #########################################################
c     ##                                                     ##
c     ##  subroutine test_tortor  --  torsion-torsion tests  ##
c     ##                                                     ##
c     #########################################################
c
c
c     "test_tortor" checks the torsion-torsion case exercised by
c     the tinker-gpu tortor.cpp test
c
c
      subroutine test_tortor
      use action
      use atoms
      use energi
      use tortor
      ...
      character*(*) tname
      character*(*) tpre
      parameter (tname='tortor_trpcage')
      parameter (tpre='test_')
c
c
      if (skiptest(tpre//tname,'amoeba'))  return
      call pushdir ('file/tortor')
      call loadfix ('trpcage','trpcage.key')
      allocate (derivs(3,n))
      allocate (refg(3,n))
      call refpath ('tortor','tortor.txt',rpath)
      call load_ref (rpath,n,ref_e,ref_ei,refv,refg,nat)
      call load_engcnt (rpath,'Torsion-Torsion',refeng,refcnt)
      eps_e = 1.0d-4
      eps_g = 1.0d-4
      eps_v = 1.0d-3
c
c     setup and level 0  --  total torsion-torsion energy
c
      call assert_int (ntortor,refcnt,tpre//tname//' count')
      e = energy ()
      call assert_real (esum,ref_e,eps_e,tpre//tname//' energy (v0)')
      call assert_real (ett,refeng,eps_e,tpre//tname//' tortor (v0)')
c

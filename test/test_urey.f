...
c     ####################################################
c     ##                                                ##
c     ##  subroutine test_urey  --  Urey-Bradley tests  ##
c     ##                                                ##
c     ####################################################
c
c
c     "test_urey" checks the Urey-Bradley case exercised by the
c     tinker-gpu urey.cpp test
c
c
      subroutine test_urey
      use action
      use atoms
      use energi
      use urey
      ...
      character*(*) tname
      character*(*) tpre
      parameter (tname='urey_h2o10')
      parameter (tpre='test_')
c
c
      if (skiptest(tpre//tname,'amoeba'))  return
      call pushdir ('file/urey')
      call loadfix ('h2o10','h2o10.key')
      allocate (derivs(3,n))
      allocate (refg(3,n))
      call refpath ('urey','urey.txt',rpath)
      call load_ref (rpath,n,ref_e,ref_ei,refv,refg,nat)
      call load_engcnt (rpath,'Urey-Bradley',refeng,refcnt)
      eps_e = 1.0d-4
      eps_g = 1.0d-4
      eps_v = 1.0d-3
c
c     setup and level 0  --  total Urey-Bradley energy
c
      call assert_int (nurey,refcnt,tpre//tname//' count')
      e = energy ()
      call assert_real (esum,ref_e,eps_e,tpre//tname//' energy (v0)')
      call assert_real (eub,refeng,eps_e,tpre//tname//' urey (v0)')
c
...
c
     ...
     &                  tpre//tname//' analysis (v3)')
      call check_engcnt (rpath,'Urey-Bradley',eub,neub,eps_e,
     &                   tpre//tname//' urey (v3)')
      deallocate (derivs)
      deallocate (refg)
      call popdir

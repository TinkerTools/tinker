...
c     ###########################################################
c     ##                                                       ##
c     ##  subroutine test_opbend  --  out-of-plane bend tests  ##
c     ##                                                       ##
c     ###########################################################
c
c
c     "test_opbend" checks the out-of-plane bend case exercised by
c     the tinker-gpu opbend.cpp test
c
c
      subroutine test_opbend
      use action
      use atoms
      use energi
      use opbend
      ...
      character*(*) tname
      character*(*) tpre
      parameter (tname='opbend_trpcage')
      parameter (tpre='test_')
c
c
      if (skiptest(tpre//tname,'amoeba'))  return
      call pushdir ('file/opbend')
      call loadfix ('trpcage','trpcage.key')
      allocate (derivs(3,n))
      allocate (refg(3,n))
      call refpath ('opbend','opbend.txt',rpath)
      call load_ref (rpath,n,ref_e,ref_ei,refv,refg,nat)
      call load_engcnt (rpath,'Out-of-Plane Bend',refeng,refcnt)
      eps_e = 1.0d-4
      eps_g = 4.0d-4
      eps_v = 1.0d-3
c
c     setup and level 0  --  total out-of-plane bend energy
c
      call assert_int (nopbend,refcnt,tpre//tname//' count')
      e = energy ()
      call assert_real (esum,ref_e,eps_e,tpre//tname//' energy (v0)')
      call assert_real (eopb,refeng,eps_e,tpre//tname//' opbend (v0)')
c

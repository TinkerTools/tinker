...
c     ############################################################
c     ##                                                        ##
c     ##  subroutine test_pitors  --  pi-orbital torsion tests  ##
c     ##                                                        ##
c     ############################################################
c
c
c     "test_pitors" checks the pi-orbital torsion case exercised by
c     the tinker-gpu pitors.cpp test
c
c
      subroutine test_pitors
      use action
      use atoms
      use energi
      use pitors
      ...
      character*(*) tname
      character*(*) tpre
      parameter (tname='pitors_trpcage')
      parameter (tpre='test_')
c
c
      if (skiptest(tpre//tname,'amoeba'))  return
      call pushdir ('file/pitors')
      call loadfix ('trpcage','trpcage.key')
      allocate (derivs(3,n))
      allocate (refg(3,n))
      call refpath ('pitors','pitors.txt',rpath)
      call load_ref (rpath,n,ref_e,ref_ei,refv,refg,nat)
      call load_engcnt (rpath,'Pi-Orbital Torsion',refeng,refcnt)
      eps_e = 1.0d-4
      eps_g = 1.0d-4
      eps_v = 1.0d-3
c
c     setup and level 0  --  total pi-orbital torsion energy
c
      call assert_int (npitors,refcnt,tpre//tname//' count')
      e = energy ()
      call assert_real (esum,ref_e,eps_e,tpre//tname//' energy (v0)')
      call assert_real (ept,refeng,eps_e,tpre//tname//' pitors (v0)')
c

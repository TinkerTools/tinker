...
c
c     modified from the standard TINKER version to handle MMFF
c
c
      subroutine readprm
      implicit none
...
c
c     #######################################################
c     ##  Following section is MMFF-specific declarations  ##
c     #######################################################
c
      integer k
...
c
c     ##########################################################
c     ##  Following section is MMFF-specific initializations  ##
c     ##########################################################
c
...
c
      do j = 1, 99
         do i = 1, 99
            MMFF_kb(i,j) = 1000.0d0
            MMFF_kb1(i,j) = 1000.0d0
            MMFF_b0(i,j) = 1000.0d0
            MMFF_b01(i,j) = 1000.0d0
            bci(i,j) = 1000.0d0
            bci_1(i,j) = 1000.0d0
            do k = 1, 99
               stbn_abc(k,i,j) = 1000.0d0
               stbn_cba(k,i,j) = 1000.0d0
               stbn_abc1(k,i,j) = 1000.0d0
               stbn_cba1(k,i,j) = 1000.0d0
               stbn_abc2(k,i,j) = 1000.0d0
               stbn_cba2(k,i,j) = 1000.0d0
               stbn_abc3(k,i,j) = 1000.0d0
               stbn_cba3(k,i,j) = 1000.0d0
               stbn_abc4(k,i,j) = 1000.0d0
               stbn_cba4(k,i,j) = 1000.0d0
               stbn_abc5(k,i,j) = 1000.0d0
               stbn_cba5(k,i,j) = 1000.0d0
               stbn_abc6(k,i,j) = 1000.0d0
               stbn_cba6(k,i,j) = 1000.0d0
               stbn_abc7(k,i,j) = 1000.0d0
               stbn_cba7(k,i,j) = 1000.0d0
               stbn_abc8(k,i,j) = 1000.0d0
               stbn_cba8(k,i,j) = 1000.0d0
               stbn_abc9(k,i,j) = 1000.0d0
               stbn_cba9(k,i,j) = 1000.0d0
               stbn_abc10(k,i,j) = 1000.0d0
               stbn_cba10(k,i,j) = 1000.0d0
               stbn_abc11(k,i,j) = 1000.0d0
               stbn_cba11(k,i,j) = 1000.0d0
            end do
         end do
      end do
      do j = 0, 99
         do i = 1, 99
            do k = 0, 99
               MMFF_ka(k,i,j) = 1000.0d0
               MMFF_ka1(k,i,j) = 1000.0d0
               MMFF_ka2(k,i,j) = 1000.0d0
               MMFF_ka3(k,i,j) = 1000.0d0
               MMFF_ka4(k,i,j) = 1000.0d0
               MMFF_ka5(k,i,j) = 1000.0d0
               MMFF_ka6(k,i,j) = 1000.0d0
               MMFF_ka7(k,i,j) = 1000.0d0
               MMFF_ka8(k,i,j) = 1000.0d0
               MMFF_teta0(k,i,j) = 1000.0d0
               MMFF_teta01(k,i,j) = 1000.0d0
               MMFF_teta02(k,i,j) = 1000.0d0
               MMFF_teta03(k,i,j) = 1000.0d0
               MMFF_teta04(k,i,j) = 1000.0d0
               MMFF_teta05(k,i,j) = 1000.0d0
               MMFF_teta06(k,i,j) = 1000.0d0
               MMFF_teta07(k,i,j) = 1000.0d0
               MMFF_teta08(k,i,j) = 1000.0d0
            end do
         end do
      end do
      blank = '                '
...
c
c     #############################################################
c     ##  Following sections are MMFF-specific parameter groups  ##
c     #############################################################
c
c     MMFF van der Waals parameters
c
         else if (keyword(1:10) .eq. 'MMFF94VDW ') then
            ia = 0
            rd = 0.0d0
            ep = 0.0d0
            rdn = 0.0d0
...
c
         else if (keyword(1:11) .eq. 'MMFF94BOND ') then
            ia = 0
            ib = 0
            fc = 0.0d0
            bd = 0.0d0
...
c
         else if (keyword(1:14) .eq. 'MMFF94BOND_ER ') then
            ia = 0
            ib = 0
            fc = 0.0d0
            bd = 0.0d0
...
c
         else if (keyword(1:7) .eq. 'COVRAD ') then
            ia = 0
            fc = 0.0d0
            bd = 0.0d0
            string = record(next:120)
            read (string,*,err=530,end=530)  ia,fc,bd
  530       continue
            rad0(ia) = fc
            paulel(ia) = bd
c
...
c
         else if (keyword(1:12) .eq. 'MMFF94ANGLE ') then
            ia = 0
            ib = 0
            ic = 0
            fc = 0.0d0
            ...
            at = 3
            string = record(next:120)
            read (string,*,err=540,end=540)  ia,ib,ic,fc,an1,at
  540       continue
            na = na + 1
...
c
         else if (keyword(1:13) .eq. 'MMFF94STRBND ') then
            ia = 0
            ib = 0
            ic = 0
            abc = 0.0d0
            ...
            SBT = 4
            string = record(next:120)
            read (string,*,err=550,end=550)  ia,ib,ic,abc,cba,SBT
  550       continue
            if (ia .ne. 0) then
               if (SBT .eq. 0) then
...
c
         else if (keyword(1:13) .eq. 'MMFF94OPBEND ') then
            ia = 0
            ib = 0
            ic = 0
            id = 0
            fc = 0.0d0
            string = record(next:120)
            read (string,*,err=560,end=560)  ia,ib,ic,id,fc
  560       continue
            call numeral (ia,pa,size)
            call numeral (ib,pb,size)
            call numeral (ic,pc,size)
...
c
         else if (keyword(1:14) .eq. 'MMFF94TORSION ') then
            ia = 0
            ib = 0
            ic = 0
            id = 0
            ...
            tt = 3
            string = record(next:120)
            read (string,*,err=570,end=570)  ia,ib,ic,id,(vt(j),
     &                                       st(j),ft(j),j=1,3),tt
  570       continue
            call numeral (ia,pa,size)
            call numeral (ib,pb,size)
...
c
            ...
            bt = 2
            string = record(next:120)
            read (string,*,err=580,end=580)  ia,ib,cg,bt
  580       continue
            if (ia .ne. 0) then
               if (bt .eq. 0) then
                  bci(ia,ib) = cg
...
c
            ...
            ie = 1000
            if = 0
            read (string,*,err=600,end=600)  ia,ib,ic,id,ie,if
  600       continue
            eqclass(if,1) = ia
            eqclass(if,2) = ib
            eqclass(if,3) = ic
...
c
         else if (keyword(1:14) .eq. 'MMFF94DEFSTBN ') then
            string = record(next:120)
            ia = 1000
            ib = 1000
            ic = 1000
...
c
         else if (keyword(1:11) .eq. 'MMFF94PROP ') then
            string = record(next:120)
            ia = 1000
            ib = 1000
            ic = 1000

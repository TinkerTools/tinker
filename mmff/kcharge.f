...
c
c     set and store MMFF base atomic partial charges
c
...
c
      ...
      do i = 1, nkey
         ...
         if (keyword(1:7) .eq. 'CHARGE ') then
            ...
            cg = 0.0d0
            string = record(next:120)
            read (string,*,err=80,end=80)  ia,cg
            if (ia.lt.0 .and. ia.ge.-n) then
               ia = -ia
               if (header) then
                  header = .false.
                  write (iout,60)
   60             format (/,' Additional Partial Charges for',
     &                       ' Specific Atoms :',
     &                    //,6x,'Atom',14x,'Charge',/)
               end if
               write (iout,70)  ia,cg
   70          format (4x,i6,8x,f12.4)
               pchg(ia) = cg
            end if
   80       continue
         end if

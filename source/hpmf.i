...
c     ##################################################################
c     ##                                                              ##
c     ##  hpmf.i  --  hydrophobic potential of mean force parameters  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     maxpmf     maximum number of hydrophobic PMF carbon atoms
c
c     npmf       number of hydrophobic carbon atoms in the system
c     acsa       SASA value for each hydrophobic PMF carbon atom
c     dacsa      derivative SASA value for each hydrophobic PMF atom
c     tslope     tanh slope (set very steep, default=100)
c     toffset    shift the tanh plot along the x-axis (default=6)
c     rpmf       vdw radius of each atom for HPMF calculation
...
c
      ...
      integer npmf,ipmf
      real*8 rpmf,acsa,dacsa
      common /hpmf/ rpmf(maxatm),acsa(maxatm),dacsa(maxatm,maxpmf),
     &              npmf,ipmf(maxatm)

...
c
      ...
      if ((use_mpole.or.use_polar.or.use_solv) .and. use_mlist) then
         call mlist
      end if
      if (use_polar .and. use_ulist)  then
              call ulist
      end if
      return
      end
...
c     ##############################################################
c     ##                                                          ##
c     ##  subroutine REvlist  --  get van der Waals neighbor lists  ##
c     ##                                                          ##
c     ##############################################################
c
c
c     "REvlist" performs an update or a complete rebuild of the
c     van der Waals neighbor list
c
c
      subroutine REvlist
      ...
      do i = 1, nvdw
         ...
         if (r2 .ge. lbuf2) then
            ...
            do k = i+1, nvdw
               ...
               r2 = xr*xr + yr*yr + zr*zr
               if (r2 .le. vbuf2) then
                  if (r2 .le. REvbufx) then
                     REnvlst(i) = REnvlst(i) + 1
                     REvlst(REnvlst(i),i) = k
                  else
                     nvlst(i) = nvlst(i) + 1
                     vlst(nvlst(i),i) = k
                  end if
...
!$OMP CRITICAL
                        do j = 1, REnvlst(k)
                          if (REvlst(j,k) .eq. i)  goto 20
                        end do
                        REnvlst(k) = REnvlst(k) + 1
                        REvlst(REnvlst(k),k) = i
   20                   continue
!$OMP END CRITICAL
                     else
!$OMP CRITICAL
                        do j = 1, nvlst(k)
                          if (vlst(j,k) .eq. i)  goto 25
                        end do
                        nvlst(k) = nvlst(k) + 1
                        vlst(nvlst(k),k) = i
   25                   continue
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine REvlight  --  build vdw pair list via lights  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "REvlight" performs a complete rebuild of the van der Waals
c     pair neighbor list for all sites using the method of lights
c
c
      subroutine REvlight (xred,yred,zred)
      use sizes
      use atoms
      use bound
      use cell
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine REclist  --  get partial charge neighbor lists  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "REclist" performs an update or a complete rebuild of the
c     electrostatic neighbor lists for partial charges
c
c
      subroutine REclist
      ...
      do i = 1, nion
         ...
         if (r2 .ge. lbuf2) then
            ...
            do k = i+1, nion
               ...
               r2 = xr*xr + yr*yr + zr*zr
               if (r2 .le. cbuf2) then
                  if (r2 .le. REcbufx) then
                     REnelst(i) = REnelst(i) + 1
                     REelst(REnelst(i),i) = k
                  else
                     nelst(i) = nelst(i) + 1
                     elst(nelst(i),i) = k
                  end if
...
!$OMP CRITICAL
                        do j = 1, REnelst(k)
                           if (REelst(j,k) .eq. i)  goto 20
                        end do
                        REnelst(k) = REnelst(k) + 1
                        REelst(REnelst(k),k) = i
   20                   continue
!$OMP END CRITICAL
                     else
!$OMP CRITICAL
                        do j = 1, nelst(k)
                           if (elst(j,k) .eq. i)  goto 25
                        end do
                        nelst(k) = nelst(k) + 1
                        elst(nelst(k),k) = i
   25                   continue
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine REclight  --  get partial charge list via lights  ##
c     ##                                                             ##
c     #################################################################
c
c
c     "REclight" performs a complete rebuild of the partial charge
c     pair neighbor list for all sites using the method of lights
c
c
      subroutine REclight

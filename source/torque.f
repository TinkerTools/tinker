...
c
c
      subroutine torque3 (i,trq1,trq2,frcx,frcy,frcz,demtt,deptt)
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'deriv.i'
...
c
      if (axetyp .eq. 'Z-then-X') then
         do j = 1, 3
            du = uv(j)*dphidv/(usiz*uvsin) + uw(j)*dphidw/usiz
            dv = -uv(j)*dphidu/(vsiz*uvsin)
            demtt(j,1) = demtt(j,1) + du
            demtt(j,3) = demtt(j,3) + dv
            demtt(j,2) = demtt(j,2) - du - dv
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
         end do
c
...
c
     ...
     &              / (wsiz*(ut1sin+ut2sin))
            demtt(j,1) = demtt(j,1) + du
            demtt(j,3) = demtt(j,3) + dv
            demtt(j,4) = demtt(j,4) + dw
            demtt(j,2) = demtt(j,2) - du - dv - dw
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
            frcy(j) = frcy(j) + dw
         end do
...
c
      if (axetyp .eq. 'Z-then-X') then
         do j = 1, 3
            du = uv(j)*dphidv/(usiz*uvsin) + uw(j)*dphidw/usiz
            dv = -uv(j)*dphidu/(vsiz*uvsin)
            deptt(j,1) = deptt(j,1) + du
            deptt(j,3) = deptt(j,3) + dv
            deptt(j,2) = deptt(j,2) - du - dv
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
         end do
c
...
c
     ...
     &              / (wsiz*(ut1sin+ut2sin))
            deptt(j,1) = deptt(j,1) + du
            deptt(j,3) = deptt(j,3) + dv
            deptt(j,4) = deptt(j,4) + dw
            deptt(j,2) = deptt(j,2) - du - dv - dw
            frcz(j) = frcz(j) + du
            frcx(j) = frcx(j) + dv
            frcy(j) = frcy(j) + dw
         end do
...
c     ################################################################
c     ##                                                            ##
c     ##  subroutine torque4  --  convert torque to Cartesian force  ##
c     ##                                                            ##
c     ################################################################
c
c
...
c
c
      subroutine torque4 (i,trq1,trq2,frcx,frcy,frcz,demi,depi)
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'deriv.i'
	  ...
	  real*8 demi(3,maxatm),depi(3,maxatm)
c
c
c     get the local frame type and the frame-defining atoms
c

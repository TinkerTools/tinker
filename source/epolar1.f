...
!$OMP& shared(ipole,pdamp,thole,x,y,z,rpole,n12,i12,n13,i13,n14,i14,
...
!$OMP& u2scale,u3scale,u4scale,nelst,elst,use_bounds,off2,uinp,poltyp,
!$OMP& ufld,dufld,depo,viro)
...
c
     ...
     &                   + tyzi*uinp(2,k) - tyzk*uinp(2,i)
     &                   + tzzi*uinp(3,k) - tzzk*uinp(3,i)
               depo(1,ii) = depo(1,ii) - f*depx
               depo(2,ii) = depo(2,ii) - f*depy
               depo(3,ii) = depo(3,ii) - f*depz
               depo(1,kk) = depo(1,kk) + f*depx
               depo(2,kk) = depo(2,kk) + f*depy
               depo(3,kk) = depo(3,kk) + f*depz
c
c     get the dEp/dR field gradient for direct polarization force
c
               term1 = psc3*(rr3-rr5*xr*xr) - prc3(1)*xr
...
c
     ...
     &                + (qixx-qiyy)*dufld(2,i)
         ...
         trqi(2) = f * trqi(2)
         trqi(3) = f * trqi(3)
         call torque4 (i,trqi,frcxi,frcyi,frczi,depo)
      end do
c
...
c
     ...
     &            + 2.0d0*qiyz*dufld(5,i) + 2.0d0*qizz*dufld(6,i)
      ...
      viro(1,1) = viro(1,1) + f*vxx
      viro(2,1) = viro(2,1) + f*vyx
      viro(3,1) = viro(3,1) + f*vzx
      viro(1,2) = viro(1,2) + f*vyx
      viro(2,2) = viro(2,2) + f*vyy
      viro(3,2) = viro(3,2) + f*vzy
      viro(1,3) = viro(1,3) + f*vzx
      viro(2,3) = viro(2,3) + f*vzy
      viro(3,3) = viro(3,3) + f*vzz
c

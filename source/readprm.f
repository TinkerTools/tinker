...
c
            ...
            ilpr = 0
            string = record(next:240)
            read (string,*,err=500,end=500)  ia,kpr,ppr,dpr,ilpr
  500       continue
            if (ia .ne. 0) then
               pepk(ia) = kpr
               peppre(ia) = ppr
...
c
            ...
            atrn = 0.0d0
            string = record(next:240)
            read (string,*,err=510,end=510)  ia,ctrn,atrn
  510       continue
            if (ia .ne. 0) then
               ctchg(ia) = ctrn
               ctdmp(ia) = atrn
...
c
            ...
            cfb = 0.0d0
            string = record(next:240)
            read (string,*,err=520,end=520)  ia,ib,cfb
  520       continue
            call numeral (ia,pa,size)
            call numeral (ib,pb,size)
            ncfb = ncfb + 1
...
c
            ...
            cfb2 = 0.0d0
            string = record(next:240)
            read (string,*,err=530,end=530)  ia,ib,ic,cfa1,cfa2,
     &                                       cfb1,cfb2
  530       continue
            call numeral (ia,pa,size)
            call numeral (ib,pb,size)
...
c
            ...
            gkrd = 0.0d0
            string = record(next:240)
            read (string,*,err=540,end=540) ia,pbrd,csrd,gkrd
  540       continue
            if (ia .ne. 0) then
               pbr(ia) = 0.5d0 * pbrd
               csr(ia) = 0.5d0 * csrd
...
c
            ...
            rp = 0.0d0
            string = record(next:240)
            read (string,*,err=550,end=550)  ia,el,iz,rp
  550       continue
            if (ia .ne. 0) then
               electron(ia) = el
               ionize(ia) = iz
...
c
            ...
            string = record(next:240)
            read (string,*,err=600,end=600)  ib
  600       continue
            if (ia .ge. maxbio) then
               write (iout,610)
  610          format (/,' READPRM  --  Too many Biopolymer Types;',
     &                    ' Increase MAXBIO')
               call fatal
            end if
...
c
            ...
            bt = 2
            string = record(next:240)
            read (string,*,err=650,end=650)  ia,ib,fc,bd,bt
  650       continue
            nb = nb + 1
            if (bt .eq. 0) then
               mmff_kb(ia,ib) = fc
...
c
            ...
            bd = 0.0d0
            string = record(next:240)
            read (string,*,err=660,end=660)  ia,ib,fc,bd
  660       continue
            r0ref(ia,ib) = fc
            r0ref(ib,ia) = fc
            kbref(ia,ib) = bd
...
c
            ...
            at = 3
            string = record(next:240)
            read (string,*,err=670,end=670)  ia,ib,ic,fc,an1,at
  670       continue
            na = na + 1
...
c
            ...
            sbt = 4
            string = record(next:240)
            read (string,*,err=680,end=680)  ia,ib,ic,abc,cba,sbt
  680       continue
            if (ia .ne. 0) then
               if (sbt .eq. 0) then
...
c
            ...
            abc = 0.0d0
            cba = 0.0d0
            read (string,*,err=690,end=690)  ia,ib,ic,abc,cba
  690       continue
...
c
            ...
            fc = 0.0d0
            string = record(next:240)
            read (string,*,err=700,end=700)  ia,ib,ic,id,fc
  700       continue
            call numeral (ia,pa,size)
            call numeral (ib,pb,size)
            call numeral (ic,pc,size)
...
c
            ...
            tt = 3
            string = record(next:240)
            read (string,*,err=710,end=710)  ia,ib,ic,id,(vt(j),
     &                                       st(j),ft(j),j=1,3),tt
  710       continue
            call numeral (ia,pa,size)
            call numeral (ib,pb,size)
...
c
            ...
            da1 = 'C'
            string = record(next:240)
            read (string,*,err=720,end=720)  ia,rd,alphi,nni,gi,da1
  720       continue
            if (ia .ne. 0) then
               rad(ia) = rd
               g(ia) = gi
...
c
            ...
            bt = 2
            string = record(next:240)
            read (string,*,err=730,end=730)  ia,ib,cg,bt
  730       continue
            if (ia .ne. 0) then
               if (bt .eq. 0) then
                  bci(ia,ib) = cg
...
c
            ...
            ia = 0
            string = record(next:240)
            read (string,*,err=740,end=740)  ia,cg,factor
  740       continue
            if (ia .ne. 0) then
               pbci(ia) = cg
               fcadj(ia) = factor
...
c
            ...
            string = record(next:240)
            read (string,*,err=750,end=750)  ia,ib,ic,id,ie,if
  750       continue

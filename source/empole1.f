...
c     ###################################################################
c     ##                                                               ##
c     ##  subroutine empole1ax  --  double loop multipole derivatives  ##
c     ##                                                               ##
c     ###################################################################
c
c
c     "empole1ax" calculates the multipole energy and derivatives with 
c     respect to Cartesian coordinates using a pairwise double loop
c
c
      subroutine empole1ax
      use sizes
      use atoms
      use bound
      use boxes
...
c
               ...
               dem(2,ii) = dem(2,ii) + ftm2(2)
               dem(3,ii) = dem(3,ii) + ftm2(3)
               call torque4 (i,ttm2,frcxi,frcyi,frczi,dem)
c
c     increment gradient due to force and torque on second site
c
               dem(1,kk) = dem(1,kk) - ftm2(1)
               dem(2,kk) = dem(2,kk) - ftm2(2)
               dem(3,kk) = dem(3,kk) - ftm2(3)
               call torque4 (k,ttm3,frcxk,frcyk,frczk,dem)
c
c     increment the internal virial tensor components
c
               iaz = iz
               ...
               yky = y(kay) - y(kk)
               zky = z(kay) - z(kk)
               vxx = -xr*ftm2(1) + xix*frcxi(1) + xiy*frcyi(1)
     &                  + xiz*frczi(1) + xkx*frcxk(1)
     &                  + xky*frcyk(1) + xkz*frczk(1)
               vyx = -yr*ftm2(1) + yix*frcxi(1) + yiy*frcyi(1)
     &                  + yiz*frczi(1) + ykx*frcxk(1)
     &                  + yky*frcyk(1) + ykz*frczk(1)
               vzx = -zr*ftm2(1) + zix*frcxi(1) + ziy*frcyi(1)
     &                  + ziz*frczi(1) + zkx*frcxk(1)
     &                  + zky*frcyk(1) + zkz*frczk(1)
               vyy = -yr*ftm2(2) + yix*frcxi(2) + yiy*frcyi(2)
     &                  + yiz*frczi(2) + ykx*frcxk(2)
     &                  + yky*frcyk(2) + ykz*frczk(2)
               vzy = -zr*ftm2(2) + zix*frcxi(2) + ziy*frcyi(2)
     &                  + ziz*frczi(2) + zkx*frcxk(2)
     &                  + zky*frcyk(2) + zkz*frczk(2)
               vzz = -zr*ftm2(3) + zix*frcxi(3) + ziy*frcyi(3)
     &                  + ziz*frczi(3) + zkx*frcxk(3)
     &                  + zky*frcyk(3) + zkz*frczk(3)
               vir(1,1) = vir(1,1) + vxx
               vir(2,1) = vir(2,1) + vyx
               vir(3,1) = vir(3,1) + vzx
...
c     ################################################################
c     ##                                                            ##
c     ##  subroutine empole1bx  --  neighbor list multipole derivs  ##
c     ##                                                            ##
c     ################################################################
c
c
c     "empole1bx" calculates the multipole energy and derivatives
c     with respect to Cartesian coordinates using a neighbor list
c
c
      subroutine empole1bx
      use sizes
      use atoms
      use bound
      use boxes

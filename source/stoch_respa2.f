...
c     nose-like isokinetic operator
               ...
               if (XI_RESPA) call applyisok (dta)
...
c     ########################################################################
c     ##                                                                    ##
c     ##  subroutine sinr_npt  --  r-RESPA with four time step levels  ##
c     ##                                                                    ##
c     ########################################################################
c
c
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine applyisok_pres                                        ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "applyisok_pres" represents the Liouville operator iL_N in Leimkuhler, Margul, and Tuckerman (2013)
c
c
      subroutine applyisok_pres (dt)
      use sizes
      use atomid
      use atoms
      use bath
      ...
      do k = 1, nc
         do j = 1, ns
            ...
            dt8 = 0.125d0 * dts
               ...
               do inhc = 1, len_nhc
                     tempstor = q_sinr1(inhc) * v_sinr1(inhc)
     &                  * v_sinr1(inhc) - kBT
                     v_sinr2(inhc) = v_sinr2(inhc)
     &                   + dt4*tempstor/q_sinr2(inhc)
               end do 
               ...
               do inhc = 1, len_nhc
                  tempstor = tempstor + (len_fac * 
     &                 q_sinr1(inhc) * v_sinr1(inhc) * 
     &                 v_sinr1(inhc) * exp(-2.0*dt2 * v_sinr2(inhc)))
               end do
               
               tempstor = sqrt(LkT/tempstor)
               vbar = vbar * tempstor   
               do inhc = 1, len_nhc
                     v_sinr1(inhc)=v_sinr1(inhc) *
     &                   tempstor*exp(-dt2*v_sinr2(inhc))   
               end do
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine applyisok2_pres                                       ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "applyisok2_pres" represents the Liouville operator iL_v in Leimkuhler, Margul, and Tuckerman (2013)
c
c
      subroutine applyisok2_pres (dt,pres)
      use sizes
      use atomid
      use atoms
      use bath
      ...
      kBT = boltzmann*kelvin
      LkT = (dble(len_nhc))*kBT
      pres_force = (3*volbox*(pres-atmsph) + kBT)


            isoA = pres_force/LkT
            isoB = (pres_force)**2/(qbar*LkT)
            rootB = sqrt(isoB)
            arg = dt_2*rootB
            if (arg > 0.00001) then
               iso_s = (1.0/rootB)*sinh(arg) + 
     ...
     &             isoA)*dt_2 + 1.0
            ...
            vbar = (vbar + (pres_force/qbar)*iso_s)/iso_sdot
            do inhc = 1, len_nhc
               v_sinr1(inhc)=v_sinr1(inhc)/iso_sdot
            end do
...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine v2random_pres                                        ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "v2random_pres" represents the Liouville operator iL_OU in Leimkuhler, Margul, and Tuckerman (2013)
c
c
      subroutine v2random_pres (dt)
      use sizes
      use atomid
      use atoms
      use bath
      ...
      kBT = boltzmann*kelvin
      sigma = sqrt(kBT*(1.0d0-exp(-2.0d0*stoch_gamma*dt))
     &             /q_sinr2(1))
      stoch_e = exp(-stoch_gamma*dt)

            do inhc = 1, len_nhc
               v_sinr2(inhc) = v_sinr2(inhc) * stoch_e
               v_sinr2(inhc) = v_sinr2(inhc) +
     &             (sigma * normal () )
            end do
...
c
c
      subroutine grad3 (ethree,derivs,ires_short)
      use limits
      use mdstuf
      use potent
      implicit none

...
c
      if (.not. onlysave) then
         ...
         if (ltitle .eq. 0) then
            fstr = '('//atmc//')'
            write (ifrc,fstr(1:4))  nonly
         else
            fstr = '('//atmc//',2x,a)'
            write (ifrc,fstr(1:9))  nonly,title(1:ltitle)
         end if
      end if
c
...
c
     ...
     &                            (i12(j,i),j=1,k)
         ...
         do ii = 1, nonly
            ...
            k = n12(i)
            if (k .eq. 0) then
               write (ifrc,fstr) ii,name(i),(-desum(j,i),j=1,3),type(i)
            else
               write (ifrc,fstr) ii,name(i),(-desum(j,i),j=1,3),type(i),
...
c
      if (.not. onlysave) then
         ...
         write (idcd)  (real(-desum(1,ionly(i))),i=1,nonly)
         write (idcd)  (real(-desum(2,ionly(i))),i=1,nonly)
         write (idcd)  (real(-desum(3,ionly(i))),i=1,nonly)
      end if
c
c     close the output unit if opened by this routine
c

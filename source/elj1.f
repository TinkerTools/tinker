...
c
      if (use_vcorr) then
         ...
         write(6,*) "long range vdw correction"
         ev = ev + elrc
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine eljSR  --  Lennard-Jones vdw derivs via list  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "eljSR" calculates the Lennard-Jones 12-6 van der Waals energy
c     and its first derivatives using a pairwise neighbor list
c
c
      subroutine eljSR
      ...
      real*8 dedx,dedy,dedz
      real*8 rik,rik2,rik3
      real*8 res_u,ru2,ru3,ru4,ru5,res_lam,q_switch
      real*8 REcut2,REtaper2
      real*8 rik4,rik5
      real*8 taper,dtaper
      real*8 vxx,vyy,vzz
      real*8 vyx,vzx,vzy
...
c
      ...
      call switch (mode)
      REcut2 = REvdwcut**2
      REtaper2 = REvdwtaper**2
      res_lam = REvdwcut - REvdwtaper
...
c
     ...
     &                           + 3.0d0*c3*rik2 + 2.0d0*c2*rik + c1
                  ...
                  if ((rik2.le.REcut2).and.(rik2.gt.REtaper2)) then
...
c     ###############################################################
c     ##                                                           ##
c     ##  subroutine eljLR  --  Lennard-Jones vdw derivs via list  ##
c     ##                                                           ##
c     ###############################################################
c
c
c     "eljLR" calculates the Lennard-Jones 12-6 van der Waals energy
c     and its first derivatives using a pairwise neighbor list
c
c
      subroutine eljLR
...
c
               if ((rik2.le.REcut2).and.(rik2.gt.REtaper2)) then
                  ...
                  de = eps * (p12-p6) * (-12.0d0/rik)
c
c     use RESPA switching if near the cutoff distance
c
                  res_u = (rik - REvdwcut + res_lam) / res_lam
                  ru2 = res_u * res_u
                  ru3 = ru2 * res_u

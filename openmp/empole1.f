...
c
     ...
     &            pscale(i14(j,ii)) = 0.5d0 * pscale(i14(j,ii))
         ...
         do k = i+1, npole
            ...
            yr = y(kk) - y(ii)
            zr = z(kk) - z(ii)
            if (use_image)  call image (xr,yr,zr)
...
c
               ...
               call torque (i,ttm2,ttm2i,frcxi,frczi)
c
...
c
               ...
               call torque (k,ttm3,ttm3i,frcxk,frczk)
c
...
c
     ...
     &                  + xkz*frczk(1) + xkx*frcxk(1) + xiz*frczi(1)
               vyx = -yr*(ftm2(1)+ftm2i(1)) + yix*frcxi(1)
     &                  + ykz*frczk(1) + ykx*frcxk(1) + yiz*frczi(1)
               vzx = -zr*(ftm2(1)+ftm2i(1)) + zix*frcxi(1)
     &                  + zkz*frczk(1) + zkx*frcxk(1) + ziz*frczi(1)
               vyy = -yr*(ftm2(2)+ftm2i(2)) + yix*frcxi(2)
     &                  + ykz*frczk(2) + ykx*frcxk(2) + yiz*frczi(2)
               vzy = -zr*(ftm2(2)+ftm2i(2)) + zix*frcxi(2)
     &                  + zkz*frczk(2) + zkx*frcxk(2) + ziz*frczi(2)
               vzz = -zr*(ftm2(3)+ftm2i(3)) + zix*frcxi(3)
     &                  + zkz*frczk(3) + zkx*frcxk(3) + ziz*frczi(3)
               vir(1,1) = vir(1,1) + vxx
               vir(2,1) = vir(2,1) + vyx
               vir(3,1) = vir(3,1) + vzx
               vir(1,2) = vir(1,2) + vyx
...
c
         ...
         do k = i, npole
            ...
            yr = y(kk) - y(ii)
            zr = z(kk) - z(ii)
            if (use_image)  call imager (xr,yr,zr,jcell)
            r2 = xr*xr + yr*yr + zr*zr
            if (r2 .le. off2) then
               r = sqrt(r2)
               ck = rpole(1,k)
...
c
      ...
      do i = 1, npole
         ...
         call torque (i,trq,trqi,frcx,frcz)
      end do
c
...
c     real*8 qgrip(2,maxgrid,maxgrid,maxgrid)
      real*8 qgrip(2,nfft1+1,nfft2+1,nfft3+1)
c     real*8, pointer :: qgrip(:,:,:,:)
c
...
c
c     dynamic allocation of the qgrip array
c
c     nullify (qgrip)
c     allocate (qgrip(2,nfft1+1,nfft2+1,nfft3+1))
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c     deallocation of the qgrip array
c
c     deallocate (qgrip)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c

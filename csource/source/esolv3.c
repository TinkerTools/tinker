/* esolv3.f -- translated by f2c (version 20050501).
...
/*     ############################################################ */
/*     ##                                                        ## */
/*     ##  subroutine esolv3  --  solvation energy and analysis  ## */
/*     ##                                                        ## */
/*     ############################################################ */


/*     "esolv3" calculates the continuum solvation energy for */
...
/* Subroutine */ int esolv3_(void)
{
    /* Format strings */
    static char fmt_10[] = "(/,\002 Individual Atomic Solvation Energy\002"
	    ",\002 Terms :\002,//,\002 Type\002,11x,\002Atom Name\002,18x,"
	    "\002Nonpolar\002,7x,\002Polar\002,6x,\002Energy\002,/)";
    static char fmt_20[] = "(\002 Solvate\002,7x,i5,\002-\002,a3,15x,3f12.4)";
    static char fmt_30[] = "(/,\002 Continuum Solvation Energy Components "
	    ":\002//,\002 Cavitation\002,26x,f12.4,/,\002 Dispersion\002,26x,"
	    "f12.4,/,\002 Electrostatic\002,23x,f12.4)";
    ...
    static doublereal e;
    static integer i__;
    static doublereal ai, rb, ri;
    extern /* Subroutine */ int egk3_(void), epb3_(void), enp3_(doublereal *, 
	    doublereal *, doublereal *, doublereal *);
    static doublereal ecav, aesa[25000];
    static logical huge__;
    static doublereal term;
    extern /* Subroutine */ int egb3b_(void), egb3a_(void);
    static doublereal aecav[25000], edisp, probe;
    static logical header;
    static doublereal aedisp[25000];
...
/*     zero out the continuum solvation energy and partitioning */
...
/*     total solvation energy for surface area only models */
	...
	surface_(&energi_1.es, analyz_1.aes, solute_1.rsolv, solute_1.asolv, &
		probe);
	action_1.nes += atoms_1.n;
...
/*     nonpolar energy as cavity formation plus dispersion */
	...
	enp3_(&ecav, aecav, &edisp, aedisp);
	energi_1.es = ecav + edisp;
	action_1.nes += atoms_1.n << 1;
	i__1 = atoms_1.n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    analyz_1.aes[i__ - 1] = aecav[i__ - 1] + aedisp[i__ - 1];
	}

/*     nonpolar energy via ACE surface area approximation */
...
/* Computing 6th power */
		...
		energi_1.es += e;
		++action_1.nes;
		analyz_1.aes[i__ - 1] += e;
	    }
	}
    }

/*     store the nonpolar or surface area energy for each atom */

    i__1 = atoms_1.n;
    for (i__ = 1; i__ <= i__1; ++i__) {
	aesa[i__ - 1] = analyz_1.aes[i__ - 1];
    }
...
/*     print a message if the energy of any interaction is large */

    header = TRUE_;
    i__1 = atoms_1.n;
    for (i__ = 1; i__ <= i__1; ++i__) {
	huge__ = (d__1 = analyz_1.aes[i__ - 1], abs(d__1)) > 25.;
	if (inform_1.debug || inform_1.verbose && huge__) {
	    ...
	    do_fio(&c__1, (char *)&aesa[i__ - 1], (ftnlen)sizeof(doublereal));
	    d__1 = analyz_1.aes[i__ - 1] - aesa[i__ - 1];
...
/*     print the components for generalized Kirkwood solvation */
...
/*     ################################################################# */
/*     ##                                                             ## */
/*     ##  subroutine egb3a  --  GB polarization energy and analysis  ## */
/*     ##                                                             ## */
/*     ################################################################# */


/*     "egb3a" calculates the generalized Born electrostatic energy */
/*     for GB/SA solvation models; also partitions the energy among */
/*     the atoms */


/* Subroutine */ int egb3a_(void)
{
    /* System generated locals */
    integer i__1, i__2;
    doublereal d__1;
...
/*     increment the overall GB polarization energy component */

		    ++action_1.nes;
		    if (i__ == k) {
			energi_1.es += e * .5;
			analyz_1.aes[i__ - 1] += e * .5;
		    } else {
			energi_1.es += e;
			analyz_1.aes[i__ - 1] += e * .5;
			analyz_1.aes[k - 1] += e * .5;
		    }
...
/*     ################################################################## */
/*     ##                                                              ## */
/*     ##  subroutine egb3b  --  GB energy and analysis for smoothing  ## */
/*     ##                                                              ## */
/*     ################################################################## */


/*     "egb3b" calculates the generalized Born electrostatuc energy */
...
/*     ############################################################## */
/*     ##                                                          ## */
/*     ##  subroutine egk3  --  GK polarization energy & analysis  ## */
/*     ##                                                          ## */
/*     ############################################################## */
...
/* Subroutine */ int egk3_(void)
{
    extern /* Subroutine */ int egk3a_(void), ediff3_(void);



/*     compute the generalized Kirkwood energy and analysis */
...
/*     ############################################################## */
/*     ##                                                          ## */
/*     ##  subroutine egk3a  --  find generalized Kirkwood energy  ## */
/*     ##                                                          ## */
/*     ############################################################## */


/*     "egk3a" calculates the electrostatic portion of the continuum */
/*     solvation energy via the generalized Kirkwood model; also */
/*     partitions the energy among the atoms */


/* Subroutine */ int egk3a_(void)
{
    /* Format strings */
    static char fmt_10[] = "(/,\002 Generalized Kirkwood Self-Energies an"
	    "d\002,\002 Cross-Energies :\002,/)";
    static char fmt_20[] = "(i8,1x,2f15.4)";
    ...
    static doublereal fq, xi, yi, zi, xr, yr, zr, gf2, gf3, gf5, gf7, rb2, 
	    gf9, xr2, yr2, zr2, uxi, uyi, uzi, uxk, uyk, uzk, dxi, dyi, dzi, 
	    dxk, dyk, dzk, rbi, rbk, gux[10], guy[10], guz[10], ewi, ewk, 
	    fgrp, expc, esym, ewii, gqxx[10], gqxy[10], qxxi, qxyi, qxzi, 
	    qyyi, qyzi, qzzi, qxxk, qxyk, qxzk, qyyk, qyzk, qzzk, gqxz[10], 
	    gqyy[10], gqyz[10], gqzz[10], ewki;
    static logical usei;
    static doublereal expc1, eself[25000], dexpc, esymi, dwater, ecross[25000]
	    ;
    extern /* Subroutine */ int switch_(char *, ftnlen), groups_(logical *, 
	    doublereal *, integer *, integer *, integer *, integer *, integer 
	    *, integer *);
    ...
    /* Fortran I/O blocks */
    static cilist io___155 = { 0, 0, 0, fmt_10, 0 };
    static cilist io___156 = { 0, 0, 0, fmt_20, 0 };
...
/*     zero out self energy and cross terms */
...
/*     increment the total GK electrostatic solvation energy */
			...
			analyz_1.aes[i__ - 1] = analyz_1.aes[i__ - 1] + e + 
				ei;
			eself[i__ - 1] = eself[i__ - 1] + e + ei;
		    } else {
			energi_1.es = energi_1.es + e + ei;
			analyz_1.aes[i__ - 1] += (e + ei) * .5;
			analyz_1.aes[k - 1] += (e + ei) * .5;
			ecross[i__ - 1] += (e + ei) * .5;
			ecross[k - 1] += (e + ei) * .5;
		    }

/*     increment the total intermolecule energy */
...
/*     print the self-energy and cross-energy terms */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    s_wsfe(&io___156);
	    do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
	    do_fio(&c__1, (char *)&eself[i__ - 1], (ftnlen)sizeof(doublereal))
		    ;
	    do_fio(&c__1, (char *)&ecross[i__ - 1], (ftnlen)sizeof(doublereal)
		    );
	    e_wsfe();
	}
    }
...
/*     ################################################################# */
/*     ##                                                             ## */
/*     ##  subroutine ediff3  --  correct GK for polarization change  ## */
/*     ##                                                             ## */
/*     ################################################################# */
...
/* Subroutine */ int ediff3_(void)
{
    ...
    static doublereal xr, yr, zr, rr1, rr3, rr5, rr7, pdi, dix, pti, diy, diz,
	     dkx, dky, dkz, qix, uix, uiy, uiz, ukx, uky, ukz, qiy, qiz, qkx, 
...
/*     increment the total GK electrostatic solvation energy */
		    ...
		    energi_1.es += ei;
		    analyz_1.aes[ii - 1] += ei * .5;
		    analyz_1.aes[kk - 1] += ei * .5;
		}
	    }
	}
...
/*     ############################################################## */
/*     ##                                                          ## */
/*     ##  subroutine epb3  --  PB polarization energy & analysis  ## */
/*     ##                                                          ## */
/*     ############################################################## */


/*     "epb3" calculates the continuum solvation energy via the */
/*     Poisson-Boltzmann plus nonpolar implicit solvation; also */
/*     partitions the energy among the atoms */


/* Subroutine */ int epb3_(void)
{
    /* System generated locals */
    integer i__1;
    ...
    extern /* Subroutine */ int ediff3_(void);
...
/*     increment solvation energy and analysis by PB results */

    energi_1.es += pb_1.pbe;
    i__1 = atoms_1.n;
    for (i__ = 1; i__ <= i__1; ++i__) {
	analyz_1.aes[i__ - 1] += pb_1.apbe[i__ - 1];
    }
...
/*     ################################################################# */
/*     ##                                                             ## */
/*     ##  subroutine enp3  --  nonpolar solvation energy & analysis  ## */
/*     ##                                                             ## */
/*     ################################################################# */


/*     "enp3" calculates the nonpolar continuum solvation energy */
/*     as a sum of cavity and dispersion terms; also partitions the */
/*     energy among the atoms */


/* Subroutine */ int enp3_(doublereal *ecav, doublereal *aecav, doublereal *
	edisp, doublereal *aedisp)
{
    /* System generated locals */
    integer i__1;
    ...
    /* Local variables */
    static integer i__;
    static doublereal reff, evol;
    extern /* Subroutine */ int ewca3_(doublereal *, doublereal *);
    static doublereal reff2, reff3, reff4, reff5, aevol, taper, esurf, aesurf[
	    25000];
...
/*     zero out the nonpolar solvation energy and partitioning */
...
/*     compute solvent excluded volume for needed for small solutes */
	...
	aevol = evol / (doublereal) atoms_1.n;
    }
...
/*     find cavity energy using both volume and SASA terms */
	...
	*ecav += taper * esurf;
	i__1 = atoms_1.n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    aecav[i__] = taper * (aevol + aesurf[i__ - 1]);
	}
...
/*     find cavity energy from only a SASA-based term */
	...
	*ecav = esurf;
	i__1 = atoms_1.n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    aecav[i__] = aesurf[i__ - 1];
	}
    }

/*     find the continuum dispersion solvation energy */

    ewca3_(edisp, &aedisp[1]);
/*     call ewca3x (edisp,aedisp) */
    return 0;
} /* enp3_ */
...
/*     ################################################################ */
/*     ##                                                            ## */
/*     ##  subroutine ewca3  --  WCA dispersion energy and analysis  ## */
/*     ##                                                            ## */
/*     ################################################################ */
...
/* Subroutine */ int ewca3_(doublereal *edisp, doublereal *aedisp)
{
    /* Format strings */
    static char fmt_10[] = "(/,\002 HCT Implicit Solvation Dispersion :\002,"
	    "/)";
    static char fmt_20[] = "(\002 Dispersion Energy for Atom\002,i6,\002 "
	    ":\002,2x,f12.4)";
    static char fmt_30[] = "(/,\002 Total HCT Dispersion Energy :\002,7x,f12"
	    ".4)";
    ...
    static doublereal r__, r2, ah, ao, ri, rk, sk, xi, yi, zi, xr, yr, zr, 
	    ...
	    uik5, lik10, iwca, lik11, lik12, uik10, uik11, uik12, irep, epsi, 
	    term, rmax, shctd, idisp, emixh, rmini, emixo, rmixh, rmixo, 
	    rmixh7, rmixo7, offset;

    /* Fortran I/O blocks */
    static cilist io___289 = { 0, 0, 0, fmt_10, 0 };
    static cilist io___290 = { 0, 0, 0, fmt_20, 0 };
    static cilist io___291 = { 0, 0, 0, fmt_30, 0 };
...
/*     zero out the WCA dispersion energy and partitioning */
...
/*     print the total dispersion energy and energy for each atom */
	...
	i__1 = atoms_1.n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    io___290.ciunit = iounit_1.iout;
	    s_wsfe(&io___290);
	    do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
	    do_fio(&c__1, (char *)&aedisp[i__], (ftnlen)sizeof(doublereal));
	    e_wsfe();
	}
...
/*     ################################################################ */
/*     ##                                                            ## */
/*     ##  subroutine ewca3x  --  alternative WCA energy & analysis  ## */
/*     ##                                                            ## */
/*     ################################################################ */
...
/* Subroutine */ int ewca3x_(doublereal *edisp, doublereal *aedisp)
{
    /* Format strings */
    static char fmt_10[] = "(/,\002 Onion Shell Implicit Solvation Dispersio"
	    "n :\002,/)";
    static char fmt_30[] = "()";
    static char fmt_40[] = "(\002 Dispersion Energy for Atom\002,i6,\002 "
	    ":\002,2x,f12.4)";
    static char fmt_50[] = "(/,\002 Total Onion Dispersion Energy :\002,5x,f"
	    "12.4)";
    ...
    /* Fortran I/O blocks */
    static cilist io___300 = { 0, 0, 0, fmt_10, 0 };
    static cilist io___322 = { 0, 0, 0, fmt_30, 0 };
    static cilist io___323 = { 0, 0, 0, fmt_40, 0 };
    static cilist io___324 = { 0, 0, 0, fmt_50, 0 };
...
/*     print header for output of the detailed energy components */

    if (inform_1.debug) {
	io___300.ciunit = iounit_1.iout;
	s_wsfe(&io___300);
	e_wsfe();
    }
...
/*     print the dispersion energy for the current shell */

/*           if (debug) then */
/*              write (iout,20)  i,inner,outer,fraction, */
/*    &                          4.0d0*pi*awater*e */
/* 20          format (' Onion Shell :',4x,i8,2f9.2,2f12.4) */
/*           end if */
	}

/* eopbend2.f -- translated by f2c (version 20050501).
...
/*     ################################################################# */
/*     ##                                                             ## */
/*     ##  subroutine eopbend2  --  out-of-plane bend Hessian; numer  ## */
/*     ##                                                             ## */
/*     ################################################################# */
...
/* Subroutine */ int eopbend2_(integer *i__)
{
    ...
    extern /* Subroutine */ int eopbend2a_(integer *);
    static integer j, k;
...
/*     find first derivatives for the base structure */
		...
		eopbend2a_(&iopbend);
		for (j = 1; j <= 3; ++j) {
		    d0_ref(j, ia) = deopb_ref(j, ia);
		    d0_ref(j, ib) = deopb_ref(j, ib);
		    d0_ref(j, ic) = deopb_ref(j, ic);
		    d0_ref(j, id) = deopb_ref(j, id);
		}
	    }
...
/*     find numerical x-components via perturbed structures */
	    ...
	    atoms_1.x[*i__ - 1] = old;
	    for (j = 1; j <= 3; ++j) {
		hessx_ref(j, ia) = hessx_ref(j, ia) + term * (deopb_ref(j, ia)
			 - d0_ref(j, ia));
		hessx_ref(j, ib) = hessx_ref(j, ib) + term * (deopb_ref(j, ib)
			 - d0_ref(j, ib));
		hessx_ref(j, ic) = hessx_ref(j, ic) + term * (deopb_ref(j, ic)
			 - d0_ref(j, ic));
		hessx_ref(j, id) = hessx_ref(j, id) + term * (deopb_ref(j, id)
			 - d0_ref(j, id));
	    }
...
/*     find numerical y-components via perturbed structures */
	    ...
	    atoms_1.y[*i__ - 1] = old;
	    for (j = 1; j <= 3; ++j) {
		hessy_ref(j, ia) = hessy_ref(j, ia) + term * (deopb_ref(j, ia)
			 - d0_ref(j, ia));
		hessy_ref(j, ib) = hessy_ref(j, ib) + term * (deopb_ref(j, ib)
			 - d0_ref(j, ib));
		hessy_ref(j, ic) = hessy_ref(j, ic) + term * (deopb_ref(j, ic)
			 - d0_ref(j, ic));
		hessy_ref(j, id) = hessy_ref(j, id) + term * (deopb_ref(j, id)
			 - d0_ref(j, id));
	    }
...
/*     find numerical z-components via perturbed structures */
	    ...
	    for (j = 1; j <= 3; ++j) {
		hessz_ref(j, ia) = hessz_ref(j, ia) + term * (deopb_ref(j, ia)
			 - d0_ref(j, ia));
		hessz_ref(j, ib) = hessz_ref(j, ib) + term * (deopb_ref(j, ib)
			 - d0_ref(j, ib));
		hessz_ref(j, ic) = hessz_ref(j, ic) + term * (deopb_ref(j, ic)
			 - d0_ref(j, ic));
		hessz_ref(j, id) = hessz_ref(j, id) + term * (deopb_ref(j, id)
			 - d0_ref(j, id));
	    }
	}
    }
...
/*     ############################################################### */
/*     ##                                                           ## */
/*     ##  subroutine eopbend2a  --  out-of-plane bend derivatives  ## */
/*     ##                                                           ## */
/*     ############################################################### */
...
/* Subroutine */ int eopbend2a_(integer *i__)
{
    /* System generated locals */
    ...
    static integer ia, ib, ic, id;
    static doublereal ee, dt, dt2, dt3, dt4, xab, yab, xia, yia, zia, xib, 
	    yib, dot, zib, xic, yic, zic, xid, yid, zid, zab, xcb, ycb, zcb, 
	    xdb, ydb, zdb, xad, yad, zad, xcd, ycd, zcd, rab2, rad2, bkk2, 
	    rdb2, rcd2, rcb2, term;
    ...
    static doublereal deddt, angle, force, dedxia, dedyia, dedcos, dedzia, 
	    ...
	    dedzid, cosine, dccdxia, dccdyia, dccdzia, dccdxic, dccdyic, 
	    dccdzic, dccdxid, dccdyid, dccdzid, deedxia, deedyia, deedzia, 
...
/*     Allinger angle between A-C-D plane and D-B vector for D-B<AC */
    ...
    } else if (s_cmp(angpot_1.opbtyp, "ALLINGER", (ftnlen)8, (ftnlen)8) == 0) 
	    {
...
/*     get the out-of-plane bending master chain rule terms */

    ee = xdb * (yab * zcb - zab * ycb) + ydb * (zab * xcb - xab * zcb) + zdb *
	     (xab * ycb - yab * xcb);
...
/* Computing MIN */
	...
	deddt = angpot_1.opbunit * force * dt * 57.29577951308232088 * (
		angpot_1.copb * 3. * dt + 2. + angpot_1.qopb * 4. * dt2 + 
		angpot_1.popb * 5. * dt3 + angpot_1.sopb * 6. * dt4);
	dedcos = -deddt * d_sign(&c_b7, &ee) / sqrt(cc * bkk2);
...
/*     chain rule terms for first derivative components */
	...
	if (s_cmp(angpot_1.opbtyp, "W-D-C", (ftnlen)8, (ftnlen)5) == 0) {
	    ...
	    dccdyid = 0.;
	    dccdzid = 0.;
	} else if (s_cmp(angpot_1.opbtyp, "ALLINGER", (ftnlen)8, (ftnlen)8) ==
		 0) {
	    term = ee / cc;
...
/*     set the out-of-plane bending derivatives */

	deopb_ref(1, ia) = dedxia;
	deopb_ref(2, ia) = dedyia;
	deopb_ref(3, ia) = dedzia;
	deopb_ref(1, ib) = dedxib;
	deopb_ref(2, ib) = dedyib;
	deopb_ref(3, ib) = dedzib;
	deopb_ref(1, ic) = dedxic;
	deopb_ref(2, ic) = dedyic;
	deopb_ref(3, ic) = dedzic;
	deopb_ref(1, id) = dedxid;
	deopb_ref(2, id) = dedyid;
	deopb_ref(3, id) = dedzid;
    }
    return 0;
} /* eopbend2a_ */

/* ebuck2.f -- translated by f2c (version 20050501).
...
/*     ############################################################## */
/*     ##                                                          ## */
/*     ##  subroutine ebuck2  --  atom-by-atom Buckingham Hessian  ## */
/*     ##                                                          ## */
/*     ############################################################## */
...
/* Subroutine */ int ebuck2_(integer *i__, doublereal *xred, doublereal *yred,
	 doublereal *zred)
{
    /* Format strings */
    static char fmt_10[] = "(/,\002 EBUCK2  --  Stophat Smoothing not Availa"
	    "ble\002,\002 for Buckingham vdw Potential\002)";

    /* Builtin functions */
    integer s_wsfe(cilist *), e_wsfe(void);

    /* Local variables */
    extern /* Subroutine */ int fatal_(void), ebuck2a_(integer *, doublereal *
	    , doublereal *, doublereal *), ebuck2b_(integer *, doublereal *, 
...
/*     choose double loop, method of lights or smoothing version */
	...
	ebuck2b_(i__, &xred[1], &yred[1], &zred[1]);
    } else {
	ebuck2a_(i__, &xred[1], &yred[1], &zred[1]);
    }
    return 0;
} /* ebuck2_ */
...
/*     ############################################################## */
/*     ##                                                          ## */
/*     ##  subroutine ebuck2a  --  double loop Buckingham Hessian  ## */
/*     ##                                                          ## */
/*     ############################################################## */
...
/* Subroutine */ int ebuck2a_(integer *iatom, doublereal *xred, doublereal *
	yred, doublereal *zred)
{
    ...
    static doublereal d2taper, expcut2, rvterm2;
...
/*     switch from exponential to R^12 at very short range */
...
/*     check for an interaction distance less than the cutoff */
		...
		if (rik2 <= shunt_1.off2) {
			...
			e = eps * (expterm - vdwpot_1.cbuck * p6);
			de = eps * (rvterm * expterm + vdwpot_1.cbuck * 6. * 
				p6 / rik);
			d2e = eps * (rvterm2 * expterm - vdwpot_1.cbuck * 42. 
				* p6 / rik2);
		    } else {
			p12 = p6 * p6;
			e = expmerge * eps * p12;
...
/*     check for an interaction distance less than the cutoff */
			...
			if (p2 <= expcut2) {
			    ...
			    expterm = vdwpot_1.abuck * exp(-vdwpot_1.bbuck / 
				    p);
			    e = eps * (expterm - vdwpot_1.cbuck * p6);
			    de = eps * (rvterm * expterm + vdwpot_1.cbuck * 
				    6. * p6 / rik);
			    d2e = eps * (rvterm2 * expterm - vdwpot_1.cbuck * 
				    42. * p6 / rik2);
			} else {
			    p12 = p6 * p6;
			    e = expmerge * eps * p12;
...
/*     ################################################################ */
/*     ##                                                            ## */
/*     ##  subroutine ebuck2b  --  Buckingham Hessian for smoothing  ## */
/*     ##                                                            ## */
/*     ################################################################ */
...
/* Subroutine */ int ebuck2b_(integer *i__, doublereal *xred, doublereal *
	yred, doublereal *zred)
{
...
/*     set coefficients for a two-Gaussian fit to MM2 vdw form */
    ...
    igauss_ref(2, 1) = 12.212189210763075;
    igauss_ref(1, 2) = -6.50376;
    igauss_ref(2, 2) = 1.997408276924538;

/*     compute Gaussian approximation to the Buckingham potential */

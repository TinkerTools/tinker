/* emm3hb1.f -- translated by f2c (version 20050501).
...
/*     ############################################################### */
/*     ##                                                           ## */
/*     ##  subroutine emm3hb1  --  MM3 vdw & hbond energy & derivs  ## */
/*     ##                                                           ## */
/*     ############################################################### */
...
/*     ################################################################# */
/*     ##                                                             ## */
/*     ##  subroutine emm3hb1a  --  double loop MM3 vdw-hbond derivs  ## */
/*     ##                                                             ## */
/*     ################################################################# */
...
/* Subroutine */ int emm3hb1a_(void)
{
    ...
    static doublereal de, p12, xi, rv, yi, zi, xr, yr, zr, xp, yp, zp, rp, 
	    rab;
    static integer iv14[25000];
    static doublereal rdn, rik, eps, dot, xia, yia, zia, xib, yib, zib, xic, 
	    yic, zic, xab, yab, zab, xcb, ycb, zcb, vxx, rab2, vyy, rcb2, vzz,
	     vyx, vzx, vzy, rik2, rik3, rik4, rik5, redi, redk, dedx, dedy, 
	    dedz, fgrp, sine, term, xred[25000], yred[25000], zred[25000];
    static logical usei;
    static doublereal ideal, deddr;
    extern /* Subroutine */ int image_(doublereal *, doublereal *, doublereal 
	    *);
    static doublereal deddt, rediv, redkv, taper, fterm, ratio, terma, termc, 
	    fcbuck, dedxia, dedyia, dedzia, dedxib, dedyib, dedzib, dedxic, 
	    dedyic, dedzic;
    static logical use_hb__;
    extern /* Subroutine */ int imager_(doublereal *, doublereal *, 
	    doublereal *, integer *);
    static doublereal vscale[25000], dtaper, cosine;
    extern /* Subroutine */ int switch_(char *, ftnlen);
    static doublereal expcut, rvterm;
...
/*     find the chain rule terms for hydrogen bonding components */

		    if (use_hb__) {
			term = eps * vdwpot_1.cbuck * p6;
			deddt *= term;
			deddr *= term;
			if (rik2 > shunt_1.cut2) {
			    deddt *= taper;
			...
			terma = deddt / (rab2 * max(rp,1e-6));
			termc = -deddt / (rcb2 * max(rp,1e-6));
...
/*     increment the derivatives for directional hydrogen bonding */

			dev_ref(1, ia) = dev_ref(1, ia) + dedxia;
			dev_ref(2, ia) = dev_ref(2, ia) + dedyia;
			dev_ref(3, ia) = dev_ref(3, ia) + dedzia;
			dev_ref(1, ib) = dev_ref(1, ib) + dedxib;
			dev_ref(2, ib) = dev_ref(2, ib) + dedyib;
			dev_ref(3, ib) = dev_ref(3, ib) + dedzib;
			dev_ref(1, ic) = dev_ref(1, ic) + dedxic;
			dev_ref(2, ic) = dev_ref(2, ic) + dedyic;
			dev_ref(3, ic) = dev_ref(3, ic) + dedzic;
		    }
...
/*     find the chain rule terms for hydrogen bonding components */
			...
			if (use_hb__) {
			    ...
			    terma = deddt / (rab2 * max(rp,1e-6));
			    termc = -deddt / (rcb2 * max(rp,1e-6));
			    dedxia = terma * (yab * zp - zab * yp) - deddr * 
				    xab;
			    dedyia = terma * (zab * xp - xab * zp) - deddr * 
				    yab;
			    dedzia = terma * (xab * yp - yab * xp) - deddr * 
				    zab;
			    dedxic = termc * (ycb * zp - zcb * yp);
			    dedyic = termc * (zcb * xp - xcb * zp);
			    dedzic = termc * (xcb * yp - ycb * xp);
...
/*     ################################################################ */
/*     ##                                                            ## */
/*     ##  subroutine emm3hb1b  --  MM3 vdw-hbond derivs via lights  ## */
/*     ##                                                            ## */
/*     ################################################################ */
...
/* Subroutine */ int emm3hb1b_(void)
{
    ...
    static doublereal eps, rdn, rik, dot, xia, yia, zia, xib, yib, zib, xic, 
	    yic, zic, xab, yab, zab, xcb, ycb, zcb, rab2, rcb2, vxx, vyy, vzz,
	     vyx, vzx, vzy, rik2, rik3, rik4, rik5, redi, redk, dedx, dedy, 
	    fgrp;
    static integer stop;
    static doublereal dedz, sine, term, xred[25000], yred[25000], zred[25000];
    static logical usei;
    static doublereal ideal, deddr, deddt, rediv, redkv, taper, terma, termc, 
	    fterm, ratio;
    static logical prime;
    static integer start;
    static doublereal xsort[200000], ysort[200000], zsort[200000], fcbuck, 
...
/*     ############################################################## */
/*     ##                                                          ## */
/*     ##  subroutine emm3hb1c  --  MM3 vdw-hbond derivs via list  ## */
/*     ##                                                          ## */
/*     ############################################################## */
...
/* Subroutine */ int emm3hb1c_(void)
{
    ...
    static doublereal deddt, rediv, redkv, taper, fterm, ratio, terma, termc, 
	    ...
	    dedyic, dedzic;
    static logical use_hb__;
    static doublereal vscale[25000], dtaper, cosine, expcut, rvterm;

/* elj2.f -- translated by f2c (version 20050501).
...
/*     ############################################################### */
/*     ##                                                           ## */
/*     ##  subroutine elj2  --  atom-by-atom Lennard-Jones Hessian  ## */
/*     ##                                                           ## */
/*     ############################################################### */
...
/* Subroutine */ int elj2_(integer *i__, doublereal *xred, doublereal *yred, 
	doublereal *zred)
{
    extern /* Subroutine */ int elj2a_(integer *, doublereal *, doublereal *, 
	    doublereal *), elj2b_(integer *, doublereal *, doublereal *, 
	    doublereal *), elj2c_(integer *, doublereal *, doublereal *, 
	    doublereal *);
...
/*     choose the method for summing over pairwise interactions */
	...
	elj2c_(i__, &xred[1], &yred[1], &zred[1]);
    } else if (warp_1.use_smooth__) {
	elj2b_(i__, &xred[1], &yred[1], &zred[1]);
    } else {
	elj2a_(i__, &xred[1], &yred[1], &zred[1]);
    }
    return 0;
} /* elj2_ */
...
/*     ############################################################### */
/*     ##                                                           ## */
/*     ##  subroutine elj2a  --  double loop Lennard-Jones Hessian  ## */
/*     ##                                                           ## */
/*     ############################################################### */
...
/* Subroutine */ int elj2a_(integer *iatom, doublereal *xred, doublereal *
	yred, doublereal *zred)
{
    ...
    static doublereal xi, yi, zi, rv, xr, yr, zr, d2e;
    static integer iv14[25000];
    static doublereal rik, eps, rik2, rik3, rik4, rik5, redi, redk, fgrp, 
	    term[9]	/* was [3][3] */;
...
/*     ################################################################# */
/*     ##                                                             ## */
/*     ##  subroutine elj2b  --  Lennard-Jones Hessian for smoothing  ## */
/*     ##                                                             ## */
/*     ################################################################# */
...
/* Subroutine */ int elj2b_(integer *i__, doublereal *xred, doublereal *yred, 
	doublereal *zred)
{
    extern /* Subroutine */ int egauss2_(integer *, doublereal *, doublereal *
	    , doublereal *);
...
/*     compute Gaussian approximation to Lennard-Jones potential */
    ...
    egauss2_(i__, &xred[1], &yred[1], &zred[1]);
    return 0;
} /* elj2b_ */
...
/*     ############################################################### */
/*     ##                                                           ## */
/*     ##  subroutine elj2c  --  Lennard-Jones Hessian for stophat  ## */
/*     ##                                                           ## */
/*     ############################################################### */
...
/* Subroutine */ int elj2c_(integer *iatom, doublereal *xred, doublereal *
	yred, doublereal *zred)
{
    /* System generated locals */
    integer i__1, i__2;
    ...
    static doublereal xi, yi, zi, rv, xr, yr, zr, d2e;
    static integer iv14[25000];
    static doublereal rik, eps, rik2, rik3, rik4, rik5, rik6, rik7, rik8, 
	    redi, redk, fgrp, term[9]	/* was [3][3] */;
    static integer list[5];
    static doublereal redi2, d2edx, d2edy, d2edz, redik, denom, rediv, redkv, 
	    width;
    static integer nlist;
    static doublereal rediv2, width2, width3, width4, width5, width6, width7, 
	    width8, vscale[25000], rediiv, redivk, redikv;
...
/* Computing 10th power */
		d__1 = denom, d__1 *= d__1, d__2 = d__1, d__1 *= d__1;
...
/*     transform the potential function via smoothing */
		...
		de -= p6 * (rik7 * 5. + rik6 * 35. * width + rik5 * 132. * 
			...
			* 64.);
		de = de * eps * p6 * 12. / (denom * 5.);
		d2e = rik6 * (rik8 * 35. + rik7 * 490. * width + rik6 * 2980. 
			* width2 + rik5 * 10280. * width3 + rik4 * 2.2e4 * 
			width4 + rik3 * 29920. * width5 + rik2 * 25280. * 
			width6 + rik * 12160. * width7 + width8 * 2560.);
		d2e -= p6 * (rik8 * 65. + rik7 * 520. * width + rik6 * 2260. *
			 width2 + rik5 * 6280. * width3 + rik4 * 11744. * 
			width4 + rik3 * 14816. * width5 + rik2 * 12160. * 
			width6 + rik * 5888. * width7 + width8 * 1280.);
		d2e = p6 * -12. * eps * d2e / (denom * 5. * rik * (rik + 
			width * 2.));
...
/*     increment diagonal and non-diagonal Hessian elements */
		...
		if (i__ == *iatom) {
		    if (i__ == iv && k == kv) {
			for (j = 1; j <= 3; ++j) {
			    hessx_ref(j, i__) = hessx_ref(j, i__) + term_ref(
				    1, j);
			    hessy_ref(j, i__) = hessy_ref(j, i__) + term_ref(
				    2, j);
			    hessz_ref(j, i__) = hessz_ref(j, i__) + term_ref(
				    3, j);
			...
			for (j = 1; j <= 3; ++j) {
			    ...
			    hessz_ref(j, k) = hessz_ref(j, k) - term_ref(3, j)
				     * redi;
			    hessx_ref(j, iv) = hessx_ref(j, iv) + term_ref(1, 
				    j) * rediiv;
			    hessy_ref(j, iv) = hessy_ref(j, iv) + term_ref(2, 
				    j) * rediiv;
			    hessz_ref(j, iv) = hessz_ref(j, iv) + term_ref(3, 
				    j) * rediiv;
			}
		    } else if (i__ == iv) {
			redk = vdw_1.kred[k - 1];
			...
			for (j = 1; j <= 3; ++j) {
			    ...
			    hessz_ref(j, k) = hessz_ref(j, k) - term_ref(3, j)
				     * redk;
			    hessx_ref(j, kv) = hessx_ref(j, kv) - term_ref(1, 
				    j) * redkv;
			    hessy_ref(j, kv) = hessy_ref(j, kv) - term_ref(2, 
				    j) * redkv;
			    hessz_ref(j, kv) = hessz_ref(j, kv) - term_ref(3, 
				    j) * redkv;
			}
		    } else {
			redk = vdw_1.kred[k - 1];

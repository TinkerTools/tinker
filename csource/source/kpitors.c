/* kpitors.f -- translated by f2c (version 20050501).
...
/* Subroutine */ int kpitors_(void)
{
    /* Format strings */
    static char fmt_20[] = "(/,\002 Additional Pi-Orbital Torsion Parameters"
	    " :\002,//,5x,\002Atom Classes\002,7x,\0022-Fold\002,/)";
    static char fmt_30[] = "(6x,2i4,4x,f12.3)";
    static char fmt_40[] = "(/,\002 KPITORS  --  Too many Pi-Orbital Torsio"
    ...
    static char pt[8];
    static integer ita, itb, npt, size, next;
    static char blank[8];
...
/*     process keywords containing pi-orbital torsion parameters */
	...
	if (s_cmp(keyword, "PITORS ", (ftnlen)7, (ftnlen)7) == 0) {
	    ia = 0;
	    ib = 0;
	    tp = 0.;
	    ...
	    i__2 = do_lio(&c__5, &c__1, (char *)&tp, (ftnlen)sizeof(
		    doublereal));
	    if (i__2 != 0) {
		goto L10;
	    }
...
L10:
	    ...
	    do_fio(&c__1, (char *)&tp, (ftnlen)sizeof(doublereal));
	    e_wsfe();
	    size = 4;
...
/* Writing concatenation */
	    ...
	    for (j = 1; j <= 500; ++j) {
		if (s_cmp(kpt_ref(0, j), blank, (ftnlen)8, (ftnlen)8) == 0 || 
			s_cmp(kpt_ref(0, j), pt, (ftnlen)8, (ftnlen)8) == 0) {
		    s_copy(kpt_ref(0, j), pt, (ftnlen)8, (ftnlen)8);
		    kpitor_1.ptcon[j - 1] = tp;
		    goto L50;
		}
	    }
...
/*     determine the total number of forcefield parameters */
	...
	if (s_cmp(kpt_ref(0, i__), blank, (ftnlen)8, (ftnlen)8) == 0) {
	    npt = i__ - 1;
	}
    }
...
/* Writing concatenation */
		...
		i__2 = npt;
		for (j = 1; j <= i__2; ++j) {
		    if (s_cmp(kpt_ref(0, j), pt, (ftnlen)8, (ftnlen)8) == 0) {
			++pitors_1.npitors;
			pitors_1.kpit[pitors_1.npitors - 1] = kpitor_1.ptcon[
				j - 1];
			ipit_ref(1, pitors_1.npitors) = i12_ref(1, ia);
			ipit_ref(2, pitors_1.npitors) = i12_ref(2, ia);
			ipit_ref(3, pitors_1.npitors) = ia;
			ipit_ref(4, pitors_1.npitors) = ib;
			ipit_ref(5, pitors_1.npitors) = i12_ref(1, ib);
			ipit_ref(6, pitors_1.npitors) = i12_ref(2, ib);
			if (i12_ref(1, ia) == ib) {
			    ipit_ref(1, pitors_1.npitors) = i12_ref(3, ia);
			}
			if (i12_ref(2, ia) == ib) {
			    ipit_ref(2, pitors_1.npitors) = i12_ref(3, ia);
			}
			if (i12_ref(1, ib) == ia) {
			    ipit_ref(5, pitors_1.npitors) = i12_ref(3, ib);
			}
			if (i12_ref(2, ib) == ia) {
			    ipit_ref(6, pitors_1.npitors) = i12_ref(3, ib);
			}
			goto L60;
		    }
		}

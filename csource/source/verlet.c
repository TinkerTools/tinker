/* verlet.f -- translated by f2c (version 20050501).
...
/*     by means of the velocity Verlet multistep recursion formula */


/* Subroutine */ int verlet_(integer *istep, doublereal *dt)
{
    ...
    static doublereal dt_2__, dt2_2__, ekin[9]	/* was [3][3] */, temp, epot, 
	    xold[25000], pres, etot, yold[25000], zold[25000], eksum;
...
/*     positions and half-step velocities via Verlet recursion */
	...
	if (usage_1.use[i__ - 1]) {
	    ...
	    atoms_1.z__[i__ - 1] = atoms_1.z__[i__ - 1] + v_ref(3, i__) * *dt 
		    + a_ref(3, i__) * dt2_2__;
	    for (j = 1; j <= 3; ++j) {
		v_ref(j, i__) = v_ref(j, i__) + a_ref(j, i__) * dt_2__;
	    }
	}
    }

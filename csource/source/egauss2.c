/* egauss2.f -- translated by f2c (version 20050501).
...
/*     ################################################################# */
/*     ##                                                             ## */
/*     ##  subroutine egauss2  --  atom-by-atom Gaussian vdw Hessian  ## */
/*     ##                                                             ## */
/*     ################################################################# */
...
/* Subroutine */ int egauss2_(integer *i__, doublereal *xred, doublereal *
	yred, doublereal *zred)
{
    extern /* Subroutine */ int egauss2a_(integer *, doublereal *, doublereal 
	    *, doublereal *), egauss2b_(integer *, doublereal *, doublereal *,
	     doublereal *);
...
/*     choose the method for summing over pairwise interactions */
	...
	egauss2b_(i__, &xred[1], &yred[1], &zred[1]);
    } else {
	egauss2a_(i__, &xred[1], &yred[1], &zred[1]);
    }
    return 0;
} /* egauss2_ */
...
/*     ################################################################# */
/*     ##                                                             ## */
/*     ##  subroutine egauss2a  --  double loop Gaussian vdw Hessian  ## */
/*     ##                                                             ## */
/*     ################################################################# */
...
/* Subroutine */ int egauss2a_(integer *iatom, doublereal *xred, doublereal *
	yred, doublereal *zred)
{
    ...
    static doublereal xi, yi, zi, xr, yr, zr, d2e;
    static integer iv14[25000];
    static doublereal rik, eps, rad2, rik2, redi, redk, fgrp, term[9]	/* 
	    was [3][3] */;
...
/* Computing 2nd power */
			...
			expterm = -b[j - 1] * rik2;
			if (expterm > expcut) {
			    expterm = a[j - 1] * b[j - 1] * exp(expterm);
			    de -= rik * 2. * expterm;
			    d2e += (b[j - 1] * 4. * rik2 - 2.) * expterm;
			}
		    }
...
/*     ############################################################### */
/*     ##                                                           ## */
/*     ##  subroutine egauss2b  --  Gaussian Hessian for smoothing  ## */
/*     ##                                                           ## */
/*     ############################################################### */
...
/* Subroutine */ int egauss2b_(integer *iatom, doublereal *xred, doublereal *
	yred, doublereal *zred)
{
    /* System generated locals */
    ...
    /* Local variables */
    static doublereal expterm2, a[10], b[10];
    static integer i__, j, k;
    static doublereal b2, t1, t2, de;
    static integer ii, kk, it, iv, kt, kv;
    static doublereal xi, yi, zi, xr, yr, zr, d2e;
    static integer iv14[25000];
    static doublereal rik, eps, rad2, rik2, redi, redk, fgrp, term[9]	/* 
	    was [3][3] */;
    static integer list[5];
    static doublereal redi2, d2edx, d2edy, d2edz, term1, term2, redik, rediv, 
	    redkv, width;
    static integer nlist;
    static doublereal wterm, rediv2, vscale[25000], rediiv, redivk, redikv, 
	    expcut;
    extern /* Subroutine */ int groups_(logical *, doublereal *, integer *, 
	    integer *, integer *, integer *, integer *, integer *);
    static logical proceed;
...
/* Computing 2nd power */
			...
			b2 = b[j - 1] * b[j - 1];
			term1 = expterm * (rik * 2. * b[j - 1] * width + 1.);
			term2 = expterm2 * (rik * 2. * b[j - 1] * width - 1.);
			de += a[j - 1] * (term1 + term2) / b2;
			term1 = b[j - 1] * 2. * width * rik * (b[j - 1] * 
				rik2 + 1.) * (expterm + expterm2);
/* Computing 2nd power */
			d__1 = b[j - 1] * width;
			term2 = (rik2 * 2. * (d__1 * d__1) + 1. + b[j - 1] * 
				rik2) * (expterm - expterm2);
			d2e += a[j - 1] * (term1 + term2) / b2;
		    }
/* Computing 3rd power */
		    d__1 = width;
		    term1 = 3. / (rik * 8. * (d__1 * (d__1 * d__1)));
		    de = -de * term1;
		    d2e = d2e * 2. * term1 / rik;
		} else {
		    if (warp_1.use_gda__) {
...
/* Computing 3rd power */
			...
			if (expterm > expcut) {
			    expterm = a[j - 1] * b[j - 1] / (t2 * t1) * exp(
				    expterm);
			    de -= rik * 2. * expterm;
			    d2e += (b[j - 1] * 4. * rik2 / t1 - 2.) * expterm;
			}
		    }
		}

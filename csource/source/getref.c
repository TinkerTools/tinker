/* getref.f -- translated by f2c (version 20050501).
...
/*     ################################################################ */
/*     ##                                                            ## */
/*     ##  subroutine getref  --  get structure from reference area  ## */
/*     ##                                                            ## */
/*     ################################################################ */
...
/* Subroutine */ int getref_(integer *iref)
{
...
/*     retrieve the filename and title line for the structure */

    s_copy(files_1.filename, reffile_ref(0, *iref), (ftnlen)120, (ftnlen)120);
    files_1.leng = refer_1.refleng[*iref - 1];
    s_copy(titles_1.title, reftitle_ref(0, *iref), (ftnlen)120, (ftnlen)120);
    titles_1.ltitle = refer_1.refltitle[*iref - 1];

/*     retrieve the coordinates, type and connectivity of each atom */

    atoms_1.n = refer_1.nref[*iref - 1];
    i__1 = atoms_1.n;
    for (i__ = 1; i__ <= i__1; ++i__) {
	s_copy(name___ref(0, i__), refnam_ref(0, i__, *iref), (ftnlen)3, (
		ftnlen)3);
	atoms_1.x[i__ - 1] = xref_ref(i__, *iref);
	atoms_1.y[i__ - 1] = yref_ref(i__, *iref);
	atoms_1.z__[i__ - 1] = zref_ref(i__, *iref);
	atoms_1.type__[i__ - 1] = reftyp_ref(i__, *iref);
	couple_1.n12[i__ - 1] = n12ref_ref(i__, *iref);
	i__2 = couple_1.n12[i__ - 1];
	for (j = 1; j <= i__2; ++j) {
	    i12_ref(j, i__) = i12ref_ref(j, i__, *iref);
	}
    }
    return 0;
} /* getref_ */
